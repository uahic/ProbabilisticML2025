...
class ConjugateFamily(ExponentialFamiliy):
    def __init__(self, likelihood: ExponentialFamiliy) -> None:
    ...
    def log_partition(self, natural_parameters: ArrayLike|jnp.ndarray,/) -> jnp.ndarray:
        ...
        if hasattr(self._likelihood, 'conjugate_log_partition')  and callable(self._likelihood.conjugate_log_partition):
                return self._likelihood.conjugate_log_partition(alpha,nu)
        else: # there is no conjugate log partition, construct the laplace approximation
            d = len(alpha)
            mode = self.mode(alpha,nu)
            prec = nu *jax.hessian(self._likelihood.log_partition, argnums=0)(jnp.asarray(mode))
            f = d/2  * (jnp.log(2*jnp.pi) - nu) - 1/2* jnp.log(jnp.linalg.det(-prec)) + jnp.inner(alpha,mode) - nu * self._likelihood.log_partition(mode)
            return f

    def mode(self, alpha: ArrayLike | jnp.ndarray, nu:float, /) -> jnp.ndarray:
        return jax.scipy.optimize.minimize(lambda x: jnp.inner(alpha,x) - nu*self._likelihood.log_partition(x),x0=jnp.zeros_like(alpha), method='L-BFGS').x
...
class BagelDistribution(ExponentialFamiliy):
    def sufficient_statistics(self, x):
        return -jnp.asarray(x) ** jnp.asarray([2,-2])
    ...
    def log_partition(self, natural_parameters):
        w = jnp.abs(jnp.asarray(w))
        return 0.5*jnp.log(jnp.pi) - jnp.log(2) - 0.5 *jnp.log(w[...,0]) -2*jnp.sqrt(w.prod(axis=-1))
